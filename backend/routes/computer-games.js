...
import * as sgdb from '../lib/art_fetchers/sgdb.js';
    ...
    try {
        const stats = await pool.query(
            `SELECT EXTRACT(year FROM initial_release_date) AS year, COUNT(*) AS count,
             ...
             FROM game_series_detailed
             WHERE dlc_for IS NULL AND initial_release_date IS NOT NULL
...
router.get('/games', async function(req, res, next) {
    try {
        ...
        games.rows.forEach(game => {
            ...
            if (!gamesByFranchise[franchise])
                gamesByFranchise[franchise] = {};
            if (!gamesByFranchise[franchise][series])
                gamesByFranchise[franchise][series] = [];
