...
export default function MovieCard(props: MovieCardProps) {
    ...
    const onDirector = (director: string) => onFilter({ type: 'SET_DIRECTOR', value: director });
        ...
        <article className="m-2 rounded-lg border border-neutral-600 bg-neutral-800 p-3 shadow-sm">
          <div className="flex gap-3">
            <div className="w-20 h-30 shrink-0 overflow-hidden rounded bg-neutral-700">
              <img src={`${API_ORIGIN}/thumbnails/movies/${movie.movie_id}.webp?v=${thumbnailVersion}`}
                   className="h-full w-full object-contain" loading="lazy" alt={`${movie.movie} cover art`}
            ...
            <div className="min-w-0 flex-1">
              ...
              <dl className="grid grid-cols-[92px_1fr] gap-x-2 gap-y-2 text-sm">
                ...
                <dd className="text-neutral-100">
                  <ClickableDate date={movie.release_date} onClick={onYear} />
                </dd>
                <dt className="text-neutral-400">Genres</dt>
                <dd className="text-neutral-100">
                  <ListOfLinks items={movie.genres} onClick={onGenre} />
                </dd>
                <dt className="text-neutral-400">Directors</dt>
                <dd className="text-neutral-100">
                  <ListOfLinks items={movie.directors} onClick={onDirector} />
                </dd>
              </dl>
            </div>
          </div>
