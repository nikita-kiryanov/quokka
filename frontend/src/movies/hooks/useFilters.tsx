...
export default function useFilters(data: MovieData | undefined) {
    ...
    const filterMovieData = (data: MovieData | undefined) => {
        ...
        if (state.director) data = filterMovies(movie => movie.directors?.includes(state.director));
        if (state.year) data = filterMovies(movie => movie.release_date.startsWith(state.year));
        if (state.genre) data = filterMovies(movie => movie.genres?.includes(state.genre));
        if (state.organization) data = filterMovies(movie => movie.organization?.includes(state.organization));
        if (completed) data = filterMovies(movie => !movie.watched);
