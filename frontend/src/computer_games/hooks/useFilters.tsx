...
export default function useFilters(data: GameData | undefined) {
    ...
    const filterGameData = (data: GameData | undefined) => {
        ...
        if (state.developer) data = filterGames(game => game.developers?.includes(state.developer));
        if (state.content) data = filterGames(game => game.content?.includes(state.content));
        if (state.year) data = filterGames(game => game.initial_release_date.startsWith(state.year));
        if (state.genre) data = filterGames(game => game.game_genres?.includes(state.genre));
        if (completed) data = filterGames(game => !game.played || !game.dlcs_played);
