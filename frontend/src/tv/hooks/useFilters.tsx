...
export default function useFilters(data: Show[] | undefined) {
    ...
    const filterTVData = (data: Show[] | undefined) => {
        ...
        let filtered = data;
        if (state.series) filtered = filtered.filter(show => show.series_name === state.series);
        if (state.genre) filtered = filtered.filter(show => show.genres?.includes(state.genre));
        if (completed) filtered = filtered.filter(show => show.bookmark != 'Finished');
