...
export default function useFilters(data: BooksData | undefined) {
    ...
    const filterBookData = (data: BooksData | undefined) => {
        ...
        if (state.author)
            data = filterBooks(book => book.authors?.includes(state.author));
        if (state.year)
            data = filterBooks(book => book.release_date.startsWith(state.year));
        if (state.genre)
            data = filterBooks(book => book.genres?.includes(state.genre));
        if (completed)
            data = filterBooks(book => !book.read);
