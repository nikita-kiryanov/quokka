...
WITH last_unwatched AS (
    ...
    SELECT show_id, COALESCE(
        array_agg(genre_name ORDER BY genre_name) FILTER (WHERE genre_name IS NOT NULL),
        ARRAY[]::text[]
    ) AS genres
    FROM shows
    LEFT JOIN shows_to_genres USING(show_id)
    GROUP BY show_id
)
SELECT show_id, series_name, show_name AS show, seasons, ended, genres,
...
LEFT JOIN genres USING(show_id)
