...
BEGIN
    ...
    IF v_data ? 'books_to_book_genres' THEN
        v_input_book_genres := ARRAY(
            SELECT DISTINCT trim(x)
            FROM jsonb_array_elements_text(
                COALESCE(NULLIF(v_data->'books_to_book_genres'->'book_genres', 'null'::jsonb), '[]'::jsonb)
            ) AS t(x)
            WHERE NULLIF(trim(x), '') IS NOT NULL
        );

        DELETE FROM books_to_book_genres btbg
        WHERE btbg.book_id = v_book_id AND NOT btbg.book_genre_name = ANY(v_input_book_genres);

        INSERT INTO books_to_book_genres (book_id, book_genre_name)
        SELECT v_book_id, book_genre_name
        FROM unnest(v_input_book_genres) AS t(book_genre_name)
        ON CONFLICT (book_id, book_genre_name) DO NOTHING;
    END IF;
